...
import { formatDisplayTime, parseAppointmentDateTime } from "../lib/appointment-time";

const router: IRouter = Router();

function toDisplayTime(requestedDate?: string | null, requestedTime?: string | null): string | null {
...
function toDisplayDate(requestedDate?: string | null): string | null {
  ...
  if (parsed.data.status === "rejected" && booking.customerId) {
    try {
      ...
      if (customer?.phone && business) {
        ...
        if (requestedDisplayDate) lines.push(`📅 Requested date: ${requestedDisplayDate}`);
...
router.post("/bookings/:id/reschedule", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (customer?.phone && business) {
      ...
      if (booking.service) lines.push(`💇 Service: ${booking.service}`);
      lines.push(`📅 New Date: *${toDisplayDate(requestedDate) ?? requestedDate}*`);
      lines.push(`🕐 New Time: *${toDisplayTime(requestedDate, requestedTime) ?? requestedTime}*`);
